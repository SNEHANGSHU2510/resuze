...
interface TailoredProjectsSectionProps {
  draft: TailoredResumeState
  updateDraft: (fn: (d: TailoredResumeState) => void) => void
}

export function TailoredProjectsSection({ draft, updateDraft }: TailoredProjectsSectionProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between mt-6">
        <label className="text-sm font-bold text-white tracking-widest uppercase">Projects ({draft.data.projects.length})</label>
        <div className="flex items-center gap-2">
          <input
            type="checkbox"
            id="jd-toggle-projects"
            checked={draft.visibility.projects}
            onChange={(e) => updateDraft((d) => { d.visibility.projects = e.target.checked })}
            className="rounded border-[var(--neon-purple)]/20 bg-background text-[var(--neon-purple)] focus:ring-[var(--neon-purple)]/50 w-4 h-4 cursor-pointer"
          />
          <label htmlFor="jd-toggle-projects" className="text-xs text-white/40 hover:text-white transition-colors cursor-pointer">Show</label>
        </div>
      </div>
        ...
        {draft.visibility.projects && (
          ...
          >
            {draft.data.projects.map((proj, projIndex) => (
              <div key={proj.id} className="space-y-3 relative group">
                <div className="absolute -left-[27px] top-2 w-3 h-3 rounded-full bg-[var(--neon-purple)]/20 border border-[var(--neon-purple)]/50 shadow-[0_0_10px_rgba(168,85,247,0.5)] transition-all group-hover:bg-[var(--neon-purple)] group-hover:scale-125" />
                <input
                  value={proj.name}
                ...
                <div className="space-y-2.5 mt-2">
                  ...
                  <button
                    onClick={() => updateDraft((d) => { d.data.projects[projIndex].bullets.push("") })}
                    className="text-[10px] font-bold tracking-widest uppercase text-[var(--neon-purple)] hover:text-white flex items-center gap-1.5 transition-colors pl-5 pt-2 hover:translate-x-1"
                  >
                    <Plus className="w-3 h-3" /> Insert Specification
                  </button>
                </div>
              </div>
            ))}
